...
def extractCosts(Variables):
    ...
    if "AREAS" in Variables['energy'].columns:
        if 'capacityCosts' in Variables.keys():
            ...
            df = df.merge(pd.DataFrame(Variables['energyCosts'].set_index(["AREAS", "TECHNOLOGIES"]) / 10 ** 9),left_on=["AREAS", "TECHNOLOGIES"], right_on=["AREAS", "TECHNOLOGIES"])
...
def extractEnergyCapacity(Variables) :
    ...
    if "AREAS" in Variables['energy'].columns:
        ...
        EnergyCapacity_df = EnergyCapacity_df.merge(pd.DataFrame(production_df.sum(axis=0) / 10 ** 6),left_on="TECHNOLOGIES", right_on="TECHNOLOGIES")
...
def EnergyAndExchange2Prod(Variables,EnergyName='energy',exchangeName='Exchange'):
    ...
    for AREA in AREAS:
        ToAREA.append(Variables["exchange"].loc[(Variables["exchange"].AREAS2 == AREA), ["Date", "exchange","AREAS1","AREAS2"]].rename(columns={"AREAS2": "AREAS"}).pivot(index=["AREAS","Date"], columns='AREAS1', values='exchange'))
...
def getMonotonesPower_multiAreas(production):
    ...
    AREAS = production.index.get_level_values("AREAS").unique()
    DATE = production.index.get_level_values("Date").unique()
    TECHNO_PROD = production.columns

    monotones_power = pd.DataFrame(
        index = pd.MultiIndex.from_product([AREAS, range(len(DATE))], names=["AREAS","HOURS"]),
        columns = TECHNO_PROD,
        dtype = "float64"
    )
...
def productionCapa_stackedBarChart(energyCapacity,capaDisp="TWh",text=False):
    ...
    - energyCapacity [AREA,TECHNOLOGIES] dataframe with [Capacity_GW,Production_TWh] columns
    ...
    dispatchTWh = energyCapacity.pivot_table(values=values, index="AREAS", columns="TECHNOLOGIES").loc[AREAS_ORDER, TECHNO_ORDER[::-1]]
    ...
    total_prod = {s:int(dispatchTWh.loc[s].sum()) for s in AREAS_ORDER}
...
def installedCapa_barChart(energyCapacity, minmaxCapacities=None, color_dict=None, **kwargs):
    ...
    capacity = pd.pivot_table(energyCapacity, values="Capacity_GW", index="AREAS", columns="TECHNOLOGIES").loc[AREAS_ORDER]

    AREAS = capacity.index
    ...
    if minmaxCapacities is not None:
        ...
        max_capacity = pd.pivot_table(minmaxCapacities, values="maxCapacity", index="AREAS", columns="TECHNOLOGIES").loc[AREAS_ORDER]/1000
        ...
        min_capacity = pd.pivot_table(minmaxCapacities, values="minCapacity", index="AREAS", columns="TECHNOLOGIES").loc[AREAS_ORDER]/1000
        ...
        data = [
            go.Bar(name=techno, x=AREAS, y=capacity[techno], text=capacity_bound_reached[techno], yaxis='y', offsetgroup=i+1, marker_color=color_dict[techno]) for i,techno in enumerate(TECHNOS)
        ]
        
    else :  

        data = [
            go.Bar(name=techno, x=AREAS, y=capacity[techno], yaxis='y', offsetgroup=i+1, marker_color=color_dict[techno]) for i,techno in enumerate(TECHNOS)
        ]
...
def installedCapaStoragePower_barChart(storage, flex_conso):
    ...
    storage = pd.concat([flex_conso, storage.drop(labels="PtGtP", axis=1)], axis=1).rename(columns={"EV":'Battery_EV'})
    ...
    data = [
        go.Bar(name=techno, x=AREAS, y=capacity[techno], yaxis='y', offsetgroup=i+1, marker_color=color_dict[techno]) for i,techno in enumerate(capacity.columns)
    ]
...
def costPerCountry(energyCapacity, cost_production, cost_storage, cost_flex):
    ...
    cost_system = pd.concat([
        ...
        cost_storage.sum(axis=1).reset_index(name="Storage").set_index("AREAS"),
        
        cost_flex.sum(axis=1).reset_index(name="Flexibility").set_index("AREAS")
        ], axis=1)

    # On se ramène à des EUR/MWh
    cost_system_normed = cost_system.divide(energyCapacity["Production_TWh"].groupby(level=[0]).sum(), axis=0).loc[AREAS_ORDER]*1e3
...
def costDecomposed_barChart(cost_production, cost_storage, cost_flex):
    ...
    cost_prod_total = pd.pivot_table(cost_production, values="Total_Milliards_euros", index="AREAS", columns="TECHNOLOGIES")
    # costs = pd.concat([cost_prod_total, cost_storage, cost_flex], axis=1).loc[AREAS_ORDER]
    ...
    data = [
        go.Bar(name=techno, x=AREAS, y=costs[techno], yaxis='y', offsetgroup=i+1, marker_color=color_dict[techno]) for i,techno in enumerate(TECHNOS)
    ]
...
def production_pieChart(production_annual, color_dict=None):
    """Plot production dsdtribution for the diffferent nodes, production_annual must be a df with multiindex (AREAS, TECHNOLOGIES) and a column called "Production_TWh"
    """
    
    # AREAS = areas_order
    # AREAS = AREAS
    AREAS = production_annual.index.get_level_values(level="AREAS").unique()

    # compute total productions
    total_prod = {area : production_annual.loc[area]["Production_TWh"].sum() for area in AREAS}

    # get color list
    if color_dict is None:
        color_dict = get_color_dict()

    color_list = get_color_list(production_annual.index.get_level_values(level="TECHNOLOGIES").unique(), color_dict=color_dict)

    # Graph generation
    specs=[[{'type':'domain'}]*len(AREAS)]
    subplots_title = [f"{area} - {int(total_prod[area])}TWh" for area in AREAS]

    fig = make_subplots(rows=1, cols=len(AREAS), specs=specs, subplot_titles=subplots_title)
    ...
    for i,area in enumerate(AREAS):
        ...
        fig.add_trace(
            go.Pie(labels=prod_area["TECHNOLOGIES"], values=prod_area["Production_TWh"], name=area, marker_colors=color_list),1,i+1
            )
...
def loadFactors(energyCapacity, title="", **kwargs):
    ...
    "TECHNOLOGIES" : ['OldNuke', 'WindOffShore', 'HydroReservoir', 'NewNuke', 'curtailment', 'CCG', 'WindOnShore', 'Biomass', 'Solar', 'TAC', 'HydroRiver', 'Coal', 'Lignite', 'CCG/TAC CH4', 'CCG/TAC H2'],
    "is_fatal" : [False, True, False, False, False, False, True, False, True, False, True, False, False, False, False]
    }).set_index("TECHNOLOGIES")
    ...
    if isinstance(energyCapacity.index, pd.MultiIndex):
        ...
        fig = px.scatter(energyCapacity.reset_index().sort_values(by=["is_fatal", "load_factor_%"], ascending=False), y="load_factor_%", x="TECHNOLOGIES", color=idx1name, symbol=idx1name, color_discrete_map=kwargs.get("color_map",None), symbol_map=kwargs.get("symbol_map",None), opacity=kwargs.get("opacity",None))
    else:
        # no AREAS index
        fig = px.scatter(energyCapacity.reset_index().sort_values(by=["is_fatal", "load_factor_%"], ascending=False), y="load_factor_%", x="TECHNOLOGIES", color_discrete_map=kwargs.get("color_map",None), symbol_map=kwargs.get("symbol_map",None), opacity=kwargs.get("opacity",None))
...
def plotProduction(df, conso=None, flex_conso=None, color_dict=None, **kwargs):
    ...
    if conso is not None:
        fig.add_trace(go.Scatter(x=conso.index, y=conso["areaConsumption"], name="Consumption", line=dict(color='black', width=2)))
