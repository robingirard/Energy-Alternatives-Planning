...
def GetElectricSystemModel_Belfort_SingleNode_H2(areaConsumption,lossesRate,availabilityFactor,
                                              TechParameters, StorageParameters,
                                              to_flex_consumption,FlexParameters,labour_ratio,
                                              H2_consumption,#H2Parameters,
                                              TimeName='Date',TechName='TECHNOLOGIES',
                                              StockTechName='STOCK_TECHNO',FlexName='FLEX_CONSUM',
                                              #H2TechName='H2_TECHNO',
                                              ...
                                              #ElectrolysisName='Electrolysis',CCH2Name='CCH2',
                                              #StorageH2Name='Storage'):
    ## Generic variables
    d1h = timedelta(hours=1)
    ...
    STOCK_TECHNO = set(StorageParameters.index.get_level_values(StockTechName).unique())
    FLEX_CONSUM = set(FlexParameters.index.get_level_values(FlexName).unique())
    #H2_TECHNO=set(H2Parameters.index.get_level_values(H2TechName).unique())
    TIME = set(areaConsumption.index.get_level_values(TimeName).unique())
    TIME_list = areaConsumption.index.get_level_values(TimeName).unique()
    ...
    TIME_df['day']=TIME_df[TimeName].apply(lambda x: x.dayofyear)
    TIME_df['day_ev']=TIME_df[TimeName].apply(lambda x: (x-12*d1h).dayofyear)
    #TIME_df['morn_ev']=TIME_df[TimeName].apply(lambda x: 1 if 8<=x.hour<=11 else 0)# 1 if morning
    #TIME_df['aft_ev']=TIME_df[TimeName].apply(lambda x: 1 if 12<=x.hour<=17 else 0)# 1 if afternoon
    ...
    model.STOCK_TECHNO = Set(initialize=STOCK_TECHNO, ordered=False)
    model.FLEX_CONSUM = Set(initialize=FLEX_CONSUM, ordered=False)
    #model.H2_TECHNO= Set(initialize=H2_TECHNO, ordered=False)
    model.TIME_TECHNOLOGIES = model.TIME * model.TECHNOLOGIES
    model.TIME_STOCK_TECHNO = model.TIME * model.STOCK_TECHNO
    model.TIME_FLEX_CONSUM = model.TIME * model.FLEX_CONSUM
    ...
    #for COLNAME in H2Parameters:
        #if COLNAME not in [H2TechName, "AREAS"]:  ### each column in TechParameters will be a parameter
            #exec("model." + COLNAME + " =          Param(model.H2_TECHNO, domain=Any,default=0," +
                 #"initialize=H2Parameters." + COLNAME + ".squeeze().to_dict())")
    ...
    ### H2 variables
    #model.power_to_H2_Pvar = Var(model.TIME,
                               #domain=NonNegativeReals)  ### Power consumed to produce hydrogen at time t
    #model.H2_to_power_Pvar = Var(model.TIME,
                                #domain=NonNegativeReals)  ### Power produced from hydrogen at time t
    # model.storageConsumption_Pvar = Var(model.TIMES, model.STOCK_TECHNO,
    # domain=NonNegativeReals)  ### Energy consumed the in a storage mean at time t (other than the one stored)
    #model.stockLevelH2_Pvar = Var(model.TIME,domain=NonNegativeReals)  ### level of the H2 stock at time t
    #model.stockLevelH2_ini_Dvar = Var(domain=NonNegativeReals)  ### initial level of H2 storage
    #model.power_to_H2_max_Dvar = Var(domain=NonNegativeReals)  # Maximum flow of power to hydrogen
    #model.H2_to_power_max_Dvar = Var(domain=NonNegativeReals)  # Maximum flow of hydrogen to power
    #model.Cmax_H2_Pvar = Var(domain=NonNegativeReals)  # Maximum capacity of hydrogen storage
    #model.H2Costs_Pvar= Var(domain=Reals)
    ...
    #def H2CostsDef_rule(model):
        #return model.power_to_H2_max_Dvar*model.capacityCostH2[ElectrolysisName]\
                #+model.H2_to_power_max_Dvar*model.capacityCostH2[CCH2Name]\
                #+model.Cmax_H2_Pvar*model.energyCostH2[StorageH2Name]==model.H2Costs_Pvar

    #model.H2_costCtr=Constraint(rule=H2CostsDef_rule)
    ### Storage constraints
    ...
    # contraintes de stock puissance
    #def H2PowerUB_rule(model, t):  # INEQ forall t
        #return model.power_to_H2_Pvar[t] - model.power_to_H2_max_Dvar <= 0

    #model.H2PowerUBCtr = Constraint(model.TIME, rule=H2PowerUB_rule)

    #def H2PowerLB_rule(model, t):  # INEQ forall t
        #return model.H2_to_power_Pvar[t] - model.H2_to_power_max_Dvar <= 0

    #model.H2PowerLBCtr = Constraint(model.TIME, rule=H2PowerLB_rule)

    # contraintes de stock capacité
    #def StockLevelH2_rule(model, t):  # EQ forall t
        #if t > TIME_list[0]:
            #return model.stockLevelH2_Pvar[t] == model.stockLevelH2_Pvar[t - d1h] * model.efficiencyH2[StorageH2Name] \
                   #+ model.power_to_H2_Pvar[t] * model.efficiencyH2[ElectrolysisName] - model.H2_to_power_Pvar[t] / \
                   #model.efficiencyH2[CCH2Name]-model.H2_consumption[t]
        #else:
            #return model.stockLevelH2_Pvar[t] == model.power_to_H2_Pvar[t] * model.efficiencyH2[
                #ElectrolysisName] - model.H2_to_power_Pvar[t] / model.efficiencyH2[CCH2Name] \
                   #- model.H2_consumption[t]\
                   #+ model.stockLevelH2_ini_Dvar * model.efficiencyH2[StorageH2Name]

    #model.StockLevelH2Ctr = Constraint(model.TIME, rule=StockLevelH2_rule)

    #def StockLevelH2_ini_rule(model):
        #return model.stockLevelH2_Pvar[TIME_list[-1]] >= model.stockLevelH2_ini_Dvar

    #model.StockLevelH2IniCtr = Constraint(rule=StockLevelH2_ini_rule)

    #def StockCapacityH2_rule(model, t):  # INEQ forall t
        #return model.stockLevelH2_Pvar[t] <= model.Cmax_H2_Pvar

    #model.StockCapacityH2Ctr = Constraint(model.TIME, rule=StockCapacityH2_rule)
