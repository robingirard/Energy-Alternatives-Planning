    ...
    match list(Set_names):
        ...
        case [*my_set_names] if allin(["FLEX_CONSUM","AREAS", 'STOCK_TECHNO'], my_set_names):
            ...
            def ObjectiveFunction_rule(model):  # OBJ
                return sum( model.energyCosts[area,tech]+\
                    model.capacityCosts[area, tech] for tech in model.TECHNOLOGIES for area in model.AREAS) + \
        ...
        case[*my_set_names] if  allin(["FLEX_CONSUM", 'STOCK_TECHNO'], my_set_names):
            ...
            def ObjectiveFunction_rule(model):  # OBJ
                return sum(model.energyCosts[tech]+\
                           model.capacityCosts[tech] for tech in model.TECHNOLOGIES) + sum(
        ...
        case [*my_set_names] if allin(["AREAS", 'STOCK_TECHNO'], my_set_names):
            ...
            def ObjectiveFunction_rule(model):  # OBJ ##TODO à corriger pour rajouter le storage cost
                return sum(
                    model.energyCosts[area,tech]+\
                     model.capacityCosts[area, tech] for tech in model.TECHNOLOGIES for area
        ...
        case _:
            ...
            def ObjectiveFunction_rule(model):  # OBJ
                return sum(model.energyCosts[tech]+\
                           ...
                           # model.energy[
                           #     t, tech] ** 2  +\
                           model.capacityCosts[tech] for tech in model.TECHNOLOGIES)
            model.OBJ = Objective(rule=ObjectiveFunction_rule, sense=minimize)
    return model
