...
def Marginal_cost_adjustment(TechParameters, number_of_sub_techs, techs, areas, carbon_tax=30, carbon_tax_ini=30,
                             gas_price_coef=1, coal_price_coef=1):
    """
    Cette fonction vient
     1 - corriger le coût marginal sans bouger la taxe carbone puis mettre éventuellement une nouvelle taxe carbone
     2 - subdiviser les technos dans techs pour avoir un nombre de sous techno  number_of_sub_techs
    :param TechParameters:
    :param number_of_sub_techs:
    :param techs:
    :param areas:
    :param carbon_tax: =30
    :param carbon_tax_ini: =30
    :param gas_price_coef: =1
    :param coal_price_coef: =1
    :return:
    """
    epsilon = 0.005
    ...
    for tech in tech_emissions.keys():
        # on applique une correction pi = (pi-taxeC*emissions)*fossil_factor+taxeC*emissions pour tenir compte d'une fraction non fossile ?
        TechParameters.loc[TechParameters.TECHNOLOGIES == tech, "energyCost"] = (TechParameters.loc[
                                                                                     TechParameters.TECHNOLOGIES == tech, "energyCost"].astype(
            float) - carbon_tax_ini * tech_emissions[tech]) * fossil_factor[tech] + carbon_tax * tech_emissions[tech]
    ...
    if number_of_sub_techs > 2:
        ...
        for tech in techs:
            for area in areas:
                ...
                if tech in ["OldNuke", "NewNuke", "Solar", 'WindOnShore', 'WindOffShore', 'HydroRiver',
                            'HydroReservoir', 'curtailment']:
                    break
                u = TechParameters[TechParameters.index == (area, tech)]
                if number_of_sub_techs % 2 != 0:

                    TechParameters.loc[(area, tech), "minCapacity"] = TechParameters.loc[
                                                                          (area, tech), "minCapacity"] / 2
                    TechParameters.loc[(area, tech), "maxCapacity"] = TechParameters.loc[
                                                                          (area, tech), "maxCapacity"] / 2
                else:
...
def Thermosensibility(areaConsumption, xls_file):
    ...
    for area in areas:
        ...
        # ConsoTempeYear_decomposed_df = ConsoSeparee_df
        NewConsoTempeYear_decomposed_df = Recompose(
            ConsoTempeYear_decomposed_df, NewThermosensibilite, TemperatureThreshold=15
        )

        u = NewConsoTempeYear_decomposed_df["Consumption"]
        u = u.reset_index()
        u["AREAS"] = area
        u.set_index(["AREAS", "Date"], inplace=True)
        u.rename(columns={"Consumption": "areaConsumption"}, inplace=True)
        areaConsumption.loc[
            (area, slice(None)), "areaConsumption"] = u  # NewConsoTempeYear_decomposed_df["Consumption"]
...
def Flexibility_data_processing(areaConsumption, year, xls_file):
    ...
    conso_type_list = ConsoParameters.FLEX_CONSUM.unique()
    # ConsoParameters_=pd.DataFrame(columns=["AREAS","FLEX_CONSUM","unit","add_consum","LoadCost","flex_ratio","flex_type","labourcost","associated_storage_cost","Smax"],data=np.array([[None]*10])).set_index(["AREAS", "FLEX_CONSUM"])
    ConsoParameters.set_index(["AREAS", "FLEX_CONSUM"], inplace=True)
    to_flex_consumption = pd.DataFrame(columns=["AREAS", "Date", "FLEX_CONSUM", "to_flex_consumption"],
                                       data=np.array([[None] * 4])).set_index(["AREAS", "Date", "FLEX_CONSUM"])

    labour_ratios = pd.DataFrame(columns=["AREAS", "Date", "FLEX_CONSUM", "labour_ratio"],
                                 data=np.array([[None] * 4])).set_index(["AREAS", "Date", "FLEX_CONSUM"])
    ...
    for area in areas_list:
        ...
        ev_consumption = NbVE * Profile2Consumption(Profile_df=VEProfile_df,
                                                    Temperature_df=ConsoTempe_df_nodup.loc[str(year)][['Temperature']])[
            ['Consumption']]
        ...
        heat_Energy = ConsoParameters.loc[(area, "HEAT"), "add_consum"] * 10 ** 6  ## Heat volume in MWh/year

        heat_Energy_flat_consumption = ev_consumption.Consumption * 0 + heat_Energy / bisextile(year)
            ...
            {'to_flex_consumption': ev_consumption.Consumption, 'FLEX_CONSUM': 'EV',
             'AREAS': area}).reset_index().set_index(['AREAS', 'Date', 'FLEX_CONSUM'])
                ...
                {'to_flex_consumption': h2_Energy_flat_consumption, 'FLEX_CONSUM': 'H2',
                 'AREAS': area}).reset_index().set_index(['AREAS', 'Date', 'FLEX_CONSUM']),
                                                                         pd.DataFrame({
                                                                                          'to_flex_consumption': heat_Energy_flat_consumption,
                                                                                          'FLEX_CONSUM': 'HEAT',
        ...
        #     to_flex_consumption.groupby(["AREAS", "FLEX_CONSUM"]).max().rename(
        #         columns={"to_flex_consumption": "max_power"}))])
    ...
    ConsoParameters.drop(["unit", "add_consum"], axis=1, inplace=True)
    ConsoParameters.dropna(inplace=True)
    ConsoParameters.reset_index(inplace=True)
    ConsoParameters.drop_duplicates(inplace=True)
    ConsoParameters.set_index(["AREAS", "FLEX_CONSUM"], inplace=True)
    ConsoParameters["max_power"] = 0
    for area in areas_list:
        for conso_type in conso_type_list:
            ConsoParameters.loc[(area, conso_type), "max_power"] = \
            to_flex_consumption.loc[(area, slice(None), conso_type)].max()[0]
    # print(4, ConsoParameters)
    to_flex_consumption.dropna(inplace=True)
    labour_ratios.dropna(inplace=True)
    return ConsoParameters, labour_ratios, to_flex_consumption
...
def CHP_processing_single_node(areaConsumption, xls_file):
    ...
    chp_production["chpProduction"] = chp_production.chpProduction.astype(float)
    areaConsumption["areaConsumption"] = areaConsumption["areaConsumption"].to_numpy() - chp_production[
        "chpProduction"].to_numpy()
    return areaConsumption
...
def Flexibility_data_processing_single_node(areaConsumption, year, xls_file):
    ...
    ConsoParameters_ = pd.DataFrame(
        columns=["FLEX_CONSUM", "unit", "add_consum", "LoadCost", "flex_ratio", "flex_type", "labourcost"],
        data=np.array([[None] * 7])).set_index(["FLEX_CONSUM"])
    ConsoParameters.set_index(["FLEX_CONSUM"], inplace=True)
    to_flex_consumption = pd.DataFrame(columns=["Date", "FLEX_CONSUM", "to_flex_consumption"],
                                       data=np.array([[None] * 3])).set_index(["Date", "FLEX_CONSUM"])
    labour_ratios = pd.DataFrame(columns=["Date", "FLEX_CONSUM", "labour_ratio"],
                                 data=np.array([[None] * 3])).set_index(["Date", "FLEX_CONSUM"])
    ...
    h2_Energy_flat_consumption = ev_consumption.Consumption * 0 + h2_Energy / bisextile(year)
    heat_Energy = ConsoParameters.loc[("HEAT"), "add_consum"] * 10 ** 6  ## Heat volume in MWh/year
    heat_Energy_flat_consumption = ev_consumption.Consumption * 0 + heat_Energy / bisextile(year)
    to_flex_consumption = pd.concat([to_flex_consumption, pd.concat([pd.DataFrame(
        {'to_flex_consumption': ev_consumption.Consumption, 'FLEX_CONSUM': 'EV'}).reset_index().set_index(
        ['Date', 'FLEX_CONSUM']), pd.DataFrame(
        {'to_flex_consumption': h2_Energy_flat_consumption, 'FLEX_CONSUM': 'H2'}).reset_index().set_index(
        ['Date', 'FLEX_CONSUM']), pd.DataFrame(
        {'to_flex_consumption': heat_Energy_flat_consumption, 'FLEX_CONSUM': 'HEAT'}).reset_index().set_index(
        ['Date', 'FLEX_CONSUM'])])])
    ConsoParameters_ = pd.concat([ConsoParameters_, ConsoParameters.join(
